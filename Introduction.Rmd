...
## More on objects in the environment
...
To clear specific objects from the workspace with the `rm()` function. For example, `remove(exam2)` would remove just the exam2 object.  

You can rename the objects in the environment with the `<-`. For example, `eaxm2fall <- exam2` would create an object called exam2fall with the exact same content as the exam2 object.   
